...
- Vite + TS + Phaser project with the core loop working end to end:
  ...
  - **Boss encounters (SHIPPED — now a MODE BREAK):** every `BOSS_EVERY`th
    ...
    (all figures cloak identical and SHUFFLE — follow him), III RETURN HIS
    FIRE (fireball tennis: timing-ring serves, tap-anywhere parry at the guard
    ring, whiff lockout, fake wind-ups, the violet lie ball, three returns to
    break the ward). Wards I/II: his cast is a visible
